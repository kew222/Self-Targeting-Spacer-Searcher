...
#Wrapper for Anti-CRISPR miner. This code takes a list of bacterial genomes and searches through each group (in group mode) or a list of individual 
#genomes (in update mode) using the anti-CRISPR miner.
...
help_message = '''
anti_CRISPR_wrapper.py uses anti_CRISPR_miner.py to search for all of the genomes in a list provided
by the user. It can function by group or just run a list of genomes. It does not support protein clustering.

Usage:
   anti_CRISPR_wrapper.py [options] [--group <groups_file> | --update <IDs_file> ]
